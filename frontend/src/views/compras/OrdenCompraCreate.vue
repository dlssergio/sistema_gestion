...
const fetchAuxiliares = async () => {
  try {
    ...
    if (tipoRes.status === 'fulfilled') {
      ...
      // Filtramos solo Ordenes
      tiposComprobante.value = data
        .filter((t) => t.nombre.toLowerCase().includes('orden'))
...
const onSelectArticulo = (val, option, index) => {
  ...
  if (art) {
    ...
    item.precio_costo_unitario = art.precio_costo
          ...
          <section class="right-panel">
            <div class="totals-widget">
              ...
              <a-button
                ...
                @click="onFinish"
                :loading="loading"
                ><SaveOutlined /> GUARDAR ORDEN</a-button
              >
            </div>
          </section>
        </main>
...
<style scoped>
/* COPIA LOS MISMOS ESTILOS DEL FACTURACREATE */
.theme-light {
  --bg-app: #f1f5f9;
