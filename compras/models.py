# compras/models.py (VERSIÓN FINAL CON VALORES POR DEFECTO)
...
class Proveedor(models.Model):
    ...
    codigo_proveedor = models.CharField(max_length=50, unique=True, blank=True, null=True, verbose_name="Código de Proveedor", help_text="Dejar en blanco para generar código automático.")
        ...
        if not self.codigo_proveedor:
            try:
                with transaction.atomic():
                    contador, created = Contador.objects.get_or_create(nombre='codigo_proveedor', defaults={'prefijo': 'P', 'ultimo_valor': 0})
...
class ComprobanteCompra(models.Model):
    ...
    class Estado(models.TextChoices): BORRADOR = 'BR', 'Borrador'; FINALIZADO = 'FN', 'Finalizado'; ANULADO = 'AN', 'Anulado'
...
class ComprobanteCompraItem(models.Model):
    ...
    precio_costo_unitario_monto = models.DecimalField(max_digits=14, decimal_places=4, verbose_name="Costo Unitario", default=0) # <-- AÑADIDO DEFAULT
    ...
    @property
    def precio_costo_unitario(self): return Money(self.precio_costo_unitario_monto, self.precio_costo_unitario_moneda.simbolo)
    @property
...
class ListaPreciosProveedor(models.Model):
    ...
    codigo = models.CharField(max_length=20, blank=True, verbose_name="Código", help_text="Código interno para identificar la lista")
    vigente_desde = models.DateField(default=timezone.now, verbose_name="Vigente Desde")
    vigente_hasta = models.DateField(null=True, blank=True, verbose_name="Vigente Hasta", help_text="Dejar en blanco si es indefinida")
    es_activa = models.BooleanField(default=True, verbose_name="¿Está Activa?")
    es_principal = models.BooleanField(default=False, verbose_name="¿Es la Lista Principal?", help_text="Solo puede haber una lista principal por proveedor")
    fecha_creacion = models.DateTimeField(auto_now_add=True)
    fecha_actualizacion = models.DateTimeField(auto_now=True)
    observaciones = models.TextField(blank=True, verbose_name="Observaciones")
    def clean(self):
        if self.vigente_hasta and self.vigente_desde > self.vigente_hasta: raise ValidationError("La fecha de fin debe ser posterior a la fecha de inicio")
        if self.es_principal:
            if ListaPreciosProveedor.objects.filter(proveedor=self.proveedor, es_principal=True).exclude(pk=self.pk).exists():
                raise ValidationError(f"El proveedor {self.proveedor} ya tiene una lista principal")
    def save(self, *args, **kwargs):
        super().save(*args, **kwargs)
        if self.es_principal:
            ListaPreciosProveedor.objects.filter(proveedor=self.proveedor, es_principal=True).exclude(pk=self.pk).update(es_principal=False)
    def __str__(self): return f"{self.proveedor.entidad.razon_social} - {self.nombre}"
    class Meta:
        verbose_name = "1. Lista de Precios de Proveedor"; verbose_name_plural = "1. Listas de Precios de Proveedores"
...
class ItemListaPreciosProveedor(models.Model):
    ...
    # <<< INICIO REFACTORIZACIÓN >>>
    precio_lista_monto = models.DecimalField(max_digits=14, decimal_places=4, verbose_name="Monto del Precio", default=0) # <-- AÑADIDO DEFAULT
    ...
    descuentos_adicionales = models.JSONField(default=list, blank=True, verbose_name="Descuentos Adicionales (%)", help_text="Lista de descuentos en cascada. Ej: [-5, -2, 1]")
    descuentos_financieros = models.JSONField(default=list, blank=True, verbose_name="Descuentos/Recargos Financieros (%)", help_text="Ej: [-10] para 10% descuento por pago contado")
    cantidad_minima = models.DecimalField(max_digits=10, decimal_places=3, default=1, verbose_name="Cantidad Mínima", help_text="Cantidad mínima para aplicar este precio")
    codigo_articulo_proveedor = models.CharField(max_length=50, blank=True, verbose_name="Código del Proveedor", help_text="Cómo el proveedor identifica este artículo")
    @property
    ...
    class Meta:
        verbose_name = "2. Ítem de Lista de Precios"; verbose_name_plural = "2. Ítems de Listas de Precios"
    ...
    if instance.pk:
        try:
            ...
            if old_instance.precio_lista != instance.precio_lista:
                # El usuario que hizo el cambio se puede obtener de un middleware,
                # pero por simplicidad lo dejamos nulo por ahora.
                HistorialPrecioProveedor.objects.create(
                    ...
                    costo_efectivo_anterior=old_instance._costo_efectivo_cache, # Usamos el cacheado
                    costo_efectivo_nuevo=None, # El nuevo se calculará post-guardado
...
def actualizar_costo_articulo_signal(sender, instance, created, **kwargs):
    ...
    actualiza el 'precio_costo' del artículo.
    """
    articulo = instance.articulo
    proveedor_de_la_lista = instance.lista_precios.proveedor

    # Comprobamos si el proveedor de la lista es el que tiene la autoridad
    if articulo.proveedor_actualiza_precio == proveedor_de_la_lista:
        costo_efectivo_actualizado = instance.costo_efectivo

        # Para evitar un bucle infinito de guardado, solo guardamos si el costo realmente cambió
...
# <<< --- FIN: ARQUITECTURA DE LISTAS DE PRECIOS --- >>>